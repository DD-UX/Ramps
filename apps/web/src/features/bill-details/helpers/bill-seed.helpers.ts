 ...
 * skeletons).
 *
 * A rail item IS the bill header (every `BillSchema` column plus the
 * vendor/entity labels and flags), so the header-level concerns — vendor,
 * entity, invoice info, PO, memo, amount, status — paint their real values
 * from it. What the list shape doesn't carry is DETAIL-ONLY data, filled here
 * with its "not loaded" value: empty collections and nulls. That's a LIE if
 * rendered as truth (an empty line-item grid reads as "this bill has no
 * lines"), which is why the screen passes the `seed` data level alongside:
 * detail-only sections (which need `full`) render their own skeletons instead
 * of these placeholders, and editing stays locked until the full record
 * replaces them.
 */
