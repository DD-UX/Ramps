...
function Label({ children }: { children: string }) {
  return <code className="w-56 flex-shrink-0 text-xs font-body text-hushed">{children}</code>;
}

/** The three weights — body 300, heading 400, bold 700. */
export const Weights: Story = {
  render: () => (
    <div className="flex max-w-2xl flex-col gap-rui-4">
      <div className="flex items-baseline gap-rui-4">
  ...
  render: () => (
    <div className="flex max-w-2xl flex-col gap-rui-4">
      ...
      <div className="flex items-baseline gap-rui-4">
        ...
        <p className="text-sm font-body text-alert">
          Ramp identified $5,660.00 of overbilling.
        </p>
      </div>
    </div>
  ),
