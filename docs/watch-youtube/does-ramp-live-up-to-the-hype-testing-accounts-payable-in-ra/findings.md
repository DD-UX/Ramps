...
accountant's review** — Financial Tech Lab / Clara CFO Group driving a _real_
Clara Media LLC tenant with a _live QuickBooks connection_, uploading _real
messy invoices_. That outsider lens is the value: we see not just the happy path
...
with its own rows (frames 18, 11). The status _is_ the primary organizing
...
- **Blocking red banner** — "Add missing information for {vendor} · This vendor
  ...
  (frame 6). Ramp explains _why_: "Due to regulatory requirements, Ramp needs to
...
- **The state transition** (frame 12) — after approving, the **same row flips to
  ...
  approval both increments the counter _and_ advances the bill to the payable
...
- **Vendor detail** (frame 16) — `Overview · Payment & tax · Insights` tabs;
  ...
  2025/2026**). Vendors carry payment config _and_ tax/compliance state.
...
- **"Send a request" modal** (frame 15) — the flow a competitor should steal:
  request missing details _from the vendor by email_, **multiple at once** via
...
(W-8 or W-9 document — required)`, plus an optional message and attachment
  ...
  banner opens: the app doesn't just flag the gap, it lets you _chase_ it.
...
## Where Ramp _fails_ — the honest limits (frames 19, 20)
...
1. **Multi-invoice-in-one-PDF** (frames 19, 4-page Home Depot Pro doc) — a single
   PDF containing _several distinct invoices_ is ingested as **one bill**. OCR
...
**What this hands us:** our simulated OCR should model _both_ outcomes honestly —
...
2. **Async ingest UX**: batch upload → toast → optimistic skeleton row →
   hydrated draft. Model the _partial-extract_ failure state explicitly.
